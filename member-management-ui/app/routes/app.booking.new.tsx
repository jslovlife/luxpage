...
export async function loader(args: LoaderFunctionArgs) {
  ...
  const packageId = url.searchParams.get("packageId") ?? SHOOT_PACKAGES[0]?.id ?? "";
...
export default function BookingNewPage() {
  const { lang, rules, packages, packageId: initialPackageId, creditsBalance, slotOptions, studios, photographerOptions, todayISO, initialMonth, maxDate } =
  ...
  const canSubmit = creditsBalance >= requiredCredits && requiredCredits > 0;
